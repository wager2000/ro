      ...
      db.serialize(function() {
        ...
        db.get(checkIfUserExists, function(err, row) {
          ...
          if (row['count(*)'] !== 0) {
            ...
            var insertUserStr = "INSERT INTO users (username, lastlogon, password) VALUES ('" + socket.username + "'," + Date.now() + ",'" + socket.username + "');"
            ...
            var queryChatsStr = "SELECT username, content FROM messages";
            // 
          ...
          db.get(lastLogoffStr, function(err, row) {
            if (err) {
              ...
              if (lastLogoff === undefined) {
                queryChatsStr = "SELECT username, content FROM messages";
...
db.serialize(function() {
  ...
  db.run('create table if not exists users (userid integer primary key, username text not null, lastlogon integer, lastlogoff integer, password text not null)');
  ...
  var insertPassword = "ALTER TABLE users ADD password text not null
  db.run(insertPassword)
...
app.post('/admin/users/clean', (req, res, next) => {
  try {
    db.serialize(function() {
      db.all("DELETE FROM users", function(err){
        res.json({ message: "table users cleaned" });
      });
    });
  } catch(err) {
...
app.post('/admin/messages/clean', (req, res, next) => {
  try {
    db.serialize(function() {
      db.all("DELETE FROM messages", function(err){
        res.json({ message: "table messages cleaned" });
      });
    });
  } catch(err) {
